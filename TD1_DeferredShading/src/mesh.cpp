...
void Mesh::load(const string &filename) {
  ...
  try {
    ...
    if (ext == "off" || ext == "OFF")
      ...
      MeshLoader::loadObj(filename, positionTmp, faceId, normalTmp, normalId,
                          texcoordTmp, texId);
    else
  ...
  if (normalId.size() ==
      0) // vertex normals not available, compute them from the faces normal
  {
    ...
    // pass 3: normalize
    for (vector<Vector3f>::iterator n_iter = normalTmp.begin();
         n_iter != normalTmp.end(); ++n_iter)
      (*n_iter).normalize();
  ...
  for (unsigned int i = 0; i < size; i++) {
    for (unsigned int j = 0; j < 3; j++) {
      ...
      _vertices.push_back(
          Vertex(Vector4f(pos[0], pos[1], pos[2], 1), color, norm, tex));
