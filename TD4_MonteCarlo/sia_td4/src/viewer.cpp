            ...
            for(int i=0; i<camera->sampleCount(); i++){
                for(int j=0; j<camera->sampleCount(); j++){
                    ...
                    if(camera->isStratified()){
                        tmp_i = Eigen::internal::random<float>(i,i+1) / camera->sampleCount();
                        tmp_j = Eigen::internal::random<float>(j,j+1) / camera->sampleCount();
                    } else{
                        tmp_i = (double)i/camera->sampleCount();
                        tmp_j = (double)j/camera->sampleCount();
                    }
                    ray.direction = (camF + camX * (2.f*float(x + 0.5f + tmp_i
                                            + offset[0])/float(camera->vpWidth())  - 1.f)
                                - camY * (2.f*float(y + 0.5f + tmp_j
                                            + offset[1])/float(camera->vpHeight()) - 1.f)).normalized();
